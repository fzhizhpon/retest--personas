...
namespace Catalogo.Infrastructure.Repositories
{
    public class EstadoCivilRepository : IEstadoCivilRepository
    {
        ...
        public EstadoCivilRepository(IConexion<OracleConnection> conexion, ILogger<EstadoCivilRepository> logger,
        ...
        public async Task<(int, IEnumerable<ComboDto>)> SelectEstadosCiviles()
        {
            try
            {
                string keyRedis = "TiposEstadosCiviles"; //LLave
                IEnumerable<ComboDto> estadosCiviles = null;
                estadosCiviles = _extensionCache.GetData<IEnumerable<ComboDto>>(keyRedis);
                ...
                if (estadosCiviles is null)
                {
                    using (var connection = _conexion.ObtenerConexion())
                    {
                        try
                        {
                            ...
                            string query = Queries.EstadoCivilQuery.SelectEstadosCiviles(_esquema);
                        ...
                        {
                            _logger.LogError($"ERROR: Error en el metodo SelectEstadosCiviles, {ex}");
                ...
                return (CodigosLogicaInterna.CODIGO_GENERICO_OK_INTERNO, estadosCiviles);
            }
            catch (Exception ex)
            {
                _logger.LogError($"ERROR: Error en el metodo SelectEstadosCiviles, {ex}");
