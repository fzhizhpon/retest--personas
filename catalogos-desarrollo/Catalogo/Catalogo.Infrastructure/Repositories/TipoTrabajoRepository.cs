...
namespace Catalogo.Infrastructure.Repositories
{
    public class TipoTrabajoRepository : ITipoTrabajoRepository
    {
        ...
        public TipoTrabajoRepository(IConexion<OracleConnection> conexion, ILogger<TipoTrabajoRepository> logger,
        ...
        public async Task<(int, IEnumerable<ComboDto>)> SelectTiposTrabajos()
        {
            try
            {
                string keyRedis = "TiposTrabajos"; //LLave
                IEnumerable<ComboDto> tipoTrabajos = null;
                tipoTrabajos = _extensionCache.GetData<IEnumerable<ComboDto>>(keyRedis);

                // Si es null, vuelve a buscar en la db
                if (tipoTrabajos is null)
                {
                    using (var connection = _conexion.ObtenerConexion())
                    {
                        try
                        {
                            ...
                            string query = Queries.TipoTrabajoQuery.SelectTiposTrabajos(_esquema);
                        ...
                        {
                            _logger.LogError($"ERROR: Error en el metodo SelectTiposTrabajos, {ex}");
                ...
                return (CodigosLogicaInterna.CODIGO_GENERICO_OK_INTERNO, tipoTrabajos);
            }
            catch (Exception ex)
            {
                _logger.LogError($"ERROR: Error en el metodo SelectTiposTrabajos, {ex}");
