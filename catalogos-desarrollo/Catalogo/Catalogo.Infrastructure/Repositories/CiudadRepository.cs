...
namespace Catalogo.Infrastructure.Repositories
{
    public class CiudadRepository : ICiudadRepository
    {
        ...
        public CiudadRepository(IConexion<OracleConnection> conexion, ILogger<CiudadRepository> logger,
        ...
        public async Task<(int, IEnumerable<ComboDto>)> SelectCiudadesPorProvincia(ObtenerCiudadProvincia dto)
        {
            try
            {
                string keyRedis = $"Ciudad-{dto.codigoPais}-{dto.codigoProvincia}"; //LLave
                IEnumerable<ComboDto> ciudades = null;
                ciudades = _extensionCache.GetData<IEnumerable<ComboDto>>(keyRedis); //Recupera de REDIS
                ...
                if (ciudades is null)
                {
                    using (var connection = _conexion.ObtenerConexion())
                    {
                        try
                        {
                            ...
                            string query = Queries.CiudadQuery.SelectCiudadPorProvincia(_esquema);
                        ...
                        {
                            _logger.LogError($"ERROR: Error en el metodo SelectCiudadesPorProvincia, {ex}");
                ...
                return (CodigosLogicaInterna.CODIGO_GENERICO_OK_INTERNO, ciudades);
            }
            catch (Exception ex)
            {
                _logger.LogError($"ERROR: Error en el metodo SelectCiudadesPorProvincia, {ex}");
