...
{
    public class OperadoraFijaService : IOperadoraFijaService
    {
        private readonly IOperadoraFijaRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public OperadoraFijaService(IOperadoraFijaRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerOperadorasFijas()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> operadoras) = await _repository.SelectOperadorasFijas();
