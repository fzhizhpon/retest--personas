...
{
    public class UsuarioDatabaseService : IUsuarioDatabaseService { 
        private readonly IUsuarioDataBaseRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public UsuarioDatabaseService(IUsuarioDataBaseRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerUsuarioDatabase()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> tiposTrabajos) = await _repository.SelectUsuarioDataBase();
