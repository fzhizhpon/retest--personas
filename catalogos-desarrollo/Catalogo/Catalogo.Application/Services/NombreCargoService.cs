...
{
    public class NombreCargosService : INombreCargosService
    {
        private readonly INombreCargosRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public NombreCargosService(INombreCargosRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerNombreCargos()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> tiposTrabajos) = await _repository.SelectNombreCargo();
