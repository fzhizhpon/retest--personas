...
{
    public class NivelInstruccionService : INivelInstruccionService
    {
        private readonly INivelInstruccionRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public NivelInstruccionService(INivelInstruccionRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerNivelesInstruccion()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> niveles) = await _repository.SelectNivelesInstruccion();
