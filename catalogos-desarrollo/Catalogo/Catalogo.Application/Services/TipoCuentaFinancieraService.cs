...
{
    public class TipoCuentaFinancieraService : ITipoCuentaFinancieraService
    {
        private readonly ITipoCuentaFinancieraRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public TipoCuentaFinancieraService(ITipoCuentaFinancieraRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerTiposCuentasFinancieras()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> tiposCuentas) = await _repository.SelectTiposCuentasFinancieras();
