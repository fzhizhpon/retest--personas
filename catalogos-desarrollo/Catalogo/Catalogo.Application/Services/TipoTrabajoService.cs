...
namespace Catalogo.Application.Services
{
    ...
    {
        private readonly ITipoTrabajoRepository _repository;
        private readonly IMensajeRespuestaRepository _mensajeRepository;

        public TipoTrabajoService(ITipoTrabajoRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerTiposTrabajos()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> tiposTrabajos) = await _repository.SelectTiposTrabajos();
