...
namespace Catalogo.Application.Services
{
    public class GeneroService : IGeneroService
    {
        ...
        public GeneroService(IGeneroRepository repository, IMensajeRespuestaRepository mensajeRepository)
        {
            _repository = repository;
            _mensajeRepository = mensajeRepository;
        }

        public async Task<Respuesta> ObtenerGeneros()
        {
            ERespuesta codigoRespuesta = ERespuesta.OK;
            int codigoEvento = 0;

            (int codigo, IEnumerable<ComboDto> generos) = await _repository.SelectGeneros();
